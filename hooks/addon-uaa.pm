...
use Genesis
  qw/bail info warning error in_array new_enough time_exec mkfile_or_fail pretty_duration run/;
...
sub cmd_details {
	return "UAA User and Group Management Addon for BOSH Genesis Kit\n" . "\n" .
...
sub perform {
	...
	$DEBUG
	  and print STDERR "DEBUG: addon-uaa.pm called with args: " .
	  join( ' ', @{ $self->{args} } ) . "\n";
	...
	bail( "#R{[ERROR]} Unknown command: $command\n" .
		  "        Run 'genesis \@ENV do uaa help' for available commands." )
	  unless $handler;
...
"        Available: add, remove, bulk-remove, list, search, activate, deactivate, reset-password, update, import, export, backup, restore, info"
	) unless $subcommand;
...
"        Available: add, list, members, add-member, remove-member, bulk-add-members, map-external, unmap-external, list-mappings"
	) unless $subcommand;
...
sub handle_clients {
	...
	bail( "#R{[ERROR]} No clients subcommand specified.\n" .
		  "        Available: add, remove, list, get, set-secret, update" )
	  unless $subcommand;
...
sub check_prerequisites {
	...
	system("command -v uaa > /dev/null 2>&1") == 0
	  or bail( "#R{[ERROR]} Command 'uaa' not found. Install it using:\n" .
...
sub get_uaa_admin_credentials {
	...
	my $admin_secret =
	  $self->vault->get( $self->env->secrets_store->base . 'uaa/clients/uaa_admin', 'secret' );
	bail( "#R{[ERROR]} Failed to read UAA admin client secret from vault.\n" .
		  "        Expected path: " .
		  $self->env->secrets_store->base .
		  'uaa/clients/uaa_admin:secret' )
	  unless $admin_secret;
...
sub uaa_login {
	...
	my $login_cmd =
	  ...
	  $creds->{client_id} . " -s " .
	  shell_quote( $creds->{client_secret} );
...
sub uaa_install {
	...
	unless ( -d $install_dir ) {
		system( "mkdir", "-p", $install_dir ) == 0
		  or bail("#R{[ERROR]} Failed to create directory: $install_dir");
	}
...
sub determine_install_dir {
	...
	my @candidate_dirs =
	  ( "$ENV{HOME}/.local/bin", "$ENV{HOME}/bin", "/usr/local/bin", "/opt/homebrew/bin" );
...
sub download_and_install {
	...
	unless ( $install_path =~ /\.exe$/ ) {
		chmod 0755, $install_path
		  or bail("#R{[ERROR]} Failed to make binary executable: $!");
	}
}
...
sub users_add {
	...
	unless ( $username =~ /^[a-zA-Z0-9._-]+$/ ) {
		bail(
"#R{[ERROR]} Invalid username format. Use only letters, numbers, dots, underscores, and hyphens."
		);
	}
	...
	my $create_cmd =
	  ...
	  " --givenName " .
	  shell_quote($given_name) .
	  " --familyName " .
	  shell_quote($family_name);
...
sub users_bulk_remove {
		...
		foreach my $line (@lines) {
			if ( $line =~ /userName:\s*(\S+)/ ) {
				...
				if ( $username =~ $pattern_regex ) {
					push @users_to_delete, $username
					  unless grep { $_ eq $username } @users_to_delete;
				}
			}
		}
	...
	describe( "#y{  Invalid/non-existent users: " . scalar(@invalid_users) . "}" )
	  if @invalid_users;
	...
	describe("");
	describe(
		$dry_run ? "#y{Users that WOULD BE deleted (dry-run mode):}" : "#R{Users to be deleted:}" );
...
sub users_reset_password {
	...
	my $user_info_cmd =
	  uaa_cmd( "get-user $username --attributes=id,userName,emails,groups,origin", 1 );
	...
	if ( !$new_password ) {
		...
		describe(
			"#y{Groups: " . ( @current_groups ? join( ", ", @current_groups ) : "none" ) . "}" );
		describe("");
	...
	my $create_cmd =
	  "create-user $username --email " .
	  shell_quote($final_email) .
	  " --password " .
	  shell_quote($new_password) .
	  " --givenName " .
	  shell_quote($given_name) .
	  " --familyName " .
	  shell_quote($family_name);
	...
	unless ( $create_result->{success} ) {
		bail( "#R{[ERROR]} Failed to recreate user $username: " .
...
sub users_search {
	...
	unless ($query_type) {
		describe("#y{Usage examples:}");
		describe(
			"  #G{users search email-domain gmail.com}          # Find users with gmail.com email");
		describe(
			"  #G{users search origin ldap}                      # Find users from LDAP origin");
		describe("  #G{users search unverified}                       # Find unverified users");
		describe(
			"  #G{users search starts-with z}                    # Find users starting with 'z'");
		describe(
			"  #G{users search created-after 2023-01-01}        # Find recently created users");
		describe("  #G{users search inactive}                         # Find inactive users");
		describe(
			"  #G{users search by-group bosh.admin}             # Find users in specific group");
...
sub users_update {
	...
	describe( "#y{  Email: " .
		  ( $new_email ? "$current_email -> $new_email" : "$final_email (unchanged)" ) . "}" );
	describe( "#y{  Password: " .
		  ( $new_password ? "*** (changed)" : "*** (new password required)" ) . "}" );
	describe( "#y{  Groups: " .
		  ( @current_groups ? join( ", ", @current_groups ) : "none" ) . " (preserved)}" );
...
sub add_user_to_group_internal {
	...
	if ( $result->{success} ) {
		...
		describe( "#y{Warning: Failed to add user $username to group $group: " .
			  $result->{output} . "}" );
	}
}
...
sub groups_add {
	...
	unless ( $groupname =~ /^[a-zA-Z0-9._-]+$/ ) {
		bail(
"#R{[ERROR]} Invalid group name format. Use only letters, numbers, dots, underscores, and hyphens."
		);
	}
...
sub groups_bulk_add_members {
	...
	describe(
		$dry_run
		? "#y{Users that WOULD BE added to group '$groupname' (dry-run mode):}"
...
sub groups_map_external {
	...
	bail("#R{[ERROR]} External group name is required for groups map-external")
	...
	my $result =
	  uaa_cmd( "map-group $uaa_group --group " .
		  shell_quote($external_group) .
		  " --origin " .
		  shell_quote($origin) );

	if ( $result->{success} ) {
		describe(
			"#G{✓} Successfully mapped external group '$external_group' to UAA group '$uaa_group'");
...
sub groups_unmap_external {
	...
	bail("#R{[ERROR]} External group name is required for groups unmap-external")
	  unless $external_group;
	...
	my $result =
	  uaa_cmd( "unmap-group $uaa_group --group " .
		  shell_quote($external_group) .
		  " --origin " .
		  shell_quote($origin) );
	...
	if ( $result->{success} ) {
		describe(
"#G{✓} Successfully unmapped external group '$external_group' from UAA group '$uaa_group'"
		);
	}
	else {
...
sub groups_list_mappings {
	...
	describe( "#G{Listing external group mappings" .
...
sub clients_add {
	...
	# Parse arguments
	my ( $client_secret, $display_name, $authorized_grant_types, $authorities, $redirect_uri,
		$scopes );
	...
	my $create_cmd =
	  ...
	  shell_quote($authorized_grant_types) .
	  " --authorities " .
	  shell_quote($authorities) .
	  " --scope " .
	  shell_quote($scopes) .
...
sub clients_remove {
	...
	# Confirm deletion
	describe(
		"#y{Are you sure you want to delete client '$client_id'? This action cannot be undone.}");
	print "Type 'yes' to confirm: ";
	my $confirmation = <STDIN>;
	chomp $confirmation;
...
sub clients_set_secret {
	...
	my $result =
	  uaa_cmd( "set-client-secret $client_id --client_secret " . shell_quote($new_secret) );
...
sub clients_update {
	...
	for ( my $i = 0 ; $i < @args ; $i++ ) {
		if ( $args[$i] =~
/^--(name|display_name|authorized_grant_types|authorities|redirect_uri|scope|access_token_validity|refresh_token_validity)$/
			&& $i + 1 < @args )
		{
...
sub users_info {
	...
	my $result = uaa_cmd(
"get-user $username --attributes=id,userName,emails,phoneNumbers,name,verified,active,origin,zoneId,passwordLastModified,previousLogonTime,lastLogonTime,groups,approvals,meta"
	);
...
sub users_backup {
	...
	my $users_result = uaa_cmd(
"list-users --count 1000 --attributes=id,userName,emails,phoneNumbers,name,verified,active,origin,groups"
	);
...
sub users_restore {
	...
	describe("");
	describe(
		"#R{WARNING: This will create new users and groups. Existing users/groups will be skipped.}"
	);
	...
	foreach my $group ( @{ $backup_data->{groups} } ) {
		...
		next
		  if $groupname =~
/^(openid|scim\.|cloud_controller\.|uaa\.|password\.|oauth\.|approvals\.|notification_preferences\.|roles$)/;
			...
			eval {
				my $result = uaa_cmd(
					"map-group " .
					  $mapping->{uaa_group} .
					  " --group " .
					  shell_quote( $mapping->{external_group} ) .
					  " --origin " .
					  shell_quote( $mapping->{origin} || 'ldap' ),
					1
				);
	...
	if ( $users_created > 0 ) {
		...
		if ($pwd_fh) {
			...
			describe(
				"#Y{          Please distribute these passwords securely and delete the file!}");
		}
	}
