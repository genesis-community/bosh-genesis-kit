...
sub perform {
	...
	# Parse options
	my %options = $self->parse_options( [ 'bosh', 'credhub', 'ssh', 'with-proxy', 'key-path=s', ] );
...
# BOSH environment variables
	if ( $options{bosh} ) {
		push @output,
		  sprintf( 'export BOSH_ENVIRONMENT=%s', _shell_quote( $exodus_data->{url} || '' ) );
		push @output,
		  sprintf( 'export BOSH_CA_CERT="$(echo -e %s)"',
			_shell_quote( $exodus_data->{ca_cert} || '' ) );
		push @output,
		  sprintf( 'export BOSH_CLIENT=%s', _shell_quote( $exodus_data->{admin_username} || '' ) );
		push @output,
		  sprintf( 'export BOSH_CLIENT_SECRET=%s',
			_shell_quote( $exodus_data->{admin_password} || '' ) );
	}

	# Credhub environment variables
	if ( $options{credhub} ) {
		push @output,
		  sprintf( 'export CREDHUB_SERVER=%s', _shell_quote( $exodus_data->{credhub_url} || '' ) );
		push @output,
		  sprintf( 'export CREDHUB_CLIENT=%s',
			_shell_quote( $exodus_data->{credhub_username} || '' ) );
		push @output,
		  sprintf( 'export CREDHUB_SECRET=%s',
			_shell_quote( $exodus_data->{credhub_password} || '' ) );
	...
	if ( $options{ssh} || $options{'with-proxy'} ) {
		...
		if ( $options{'with-proxy'} ) {
			push @output,
			  sprintf( 'export BOSH_ALL_PROXY=ssh+socks5://netop@%s:22?private-key=${ssh_key}',
				$host_addr );
		}
	}
