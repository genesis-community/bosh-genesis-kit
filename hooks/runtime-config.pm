...
sub init {
	...
	# Define valid builds
	$obj->register_runtime_config_builds( [ dns => "BOSH DNS" ], 'ops-access', 'toolbelt', );
...
sub build_dns_runtime {
				...
				jobs => [
					{
						...
						properties => {
							api => {
								client => {
									tls => {
										...
										certificate =>
										  $self->_get_secret('dns_api_tls/client:certificate'),
								...
								server => {
									tls => {
										ca => $self->_get_secret('dns_api_tls/ca:certificate'),
										certificate =>
										  $self->_get_secret('dns_api_tls/server:certificate'),
							...
							cache => {
								enabled =>
								  scalar( $self->env->lookup( 'dns_cache', JSON::PP::true ) )
							}
						}
					}
				]
	...
	if ( @$blacklist || @$ig_blacklist ) {
		...
		$runtime->{addons}[0]{exclude}{deployments} = [ map { { name => $_ } } @$blacklist ]
		  if @$blacklist;
		$runtime->{addons}[0]{exclude}{instance_groups} = [ map { { name => $_ } } @$ig_blacklist ]
		  if @$ig_blacklist;
	}
	...
	my $upstream_release = scalar(
		$self->spruce_merge(
			'--skip-eval', '--cherry-pick',
			'releases',    $self->kit->path('bosh-deployment/runtime-configs/dns.yml')
		)
	);
	...
	my ( $out, $rc, $err ) = run(
		...
		$upstream_release, $self->kit->path('overlay/releases/bosh-dns.yml')
	);
		...
		addons => [
			{
				...
				exclude => {
					jobs => [ { name => 'user_add', release => 'os-conf' } ]
				},
				jobs => [
					{
						name       => 'user_add',
	...
	my $toolbelt_runtime = {
		addons => [
			...
			},
			{
				...
				jobs => [ { name => 'toolbelt-veritas', release => 'toolbelt' } ]
			}
		]
	};
