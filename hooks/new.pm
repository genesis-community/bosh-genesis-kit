...
use Genesis::Term
  qw/prompt_for_boolean prompt_for_select prompt_for_line prompt_for prompt_for_block in_controlling_terminal/;
...
sub get_bosh_environment {
	...
	if (   $ENV{GENESIS_BOSH_ENVIRONMENT}
		&& $ENV{GENESIS_BOSH_ENVIRONMENT} ne $ENV{GENESIS_ENVIRONMENT} )
	{
		...
		if (@segments) {
			...
			my $cmd =
			  ...
			  join( " ", @segments ) .
			  " | spruce json | jq -r '.genesis.bosh_env // \"\"'";
	...
	while (1) {
		...
		describe(
"#R{[INVALID]} Target BOSH director environment must not be the same as this environment."
		);
	}
...
sub get_network_config {
	...
	my $ip = prompt_for_line( 'What static IP do you want to deploy this BOSH director on?',
		'--validation', 'ip' );
	...
	if ($is_proto) {
		my $proto_net =
		  prompt_for_line('What network should this BOSH director exist in (in CIDR notation)?');

		my $proto_gw =
		  prompt_for_line( 'What default gateway (IP address) should this BOSH director use?',
			'--validation', 'ip' );

		my @proto_dns = prompt_for( 'multi-line', 'What DNS servers should BOSH use?' );
...
sub configure_aws {
	...
	my $aws_region = prompt_for_line('What AWS region would you like to deploy to?');
	...
	if ( $aws_auth_method eq 'access_key' ) {
		my $aws_access_key =
		  prompt_for_line( 'What is your AWS Access Key?', '--echo', '--secret-line' );

		my $aws_secret_key = prompt_for_line( 'What is your AWS Secret Key?', '--secret-line' );

		# Store in vault
		my $secrets_base = $self->env->secrets_base;
		system(
"safe set --quiet \"${secrets_base}aws\" access_key=\"$aws_access_key\" secret_key=\"$aws_secret_key\""
		);
	}
	else {
	...
	if ($is_proto) {
		my $aws_subnet = prompt_for_line('What is the ID of the AWS subnet you want to deploy to?');
...
sub configure_vsphere {
	...
	if ( !$is_proto ) {
		if (
			system(
"safe get \"${ENV{GENESIS_EXODUS_MOUNT}}/$ENV{BOSH_ALIAS}/bosh:vault_base\" 2>/dev/null"
			) == 0
		  )
		{
			my $bosh_vault =
			...
			if ($json) {
				...
				if ($parent_env_json) {
					...
					$parent_vsphere_datacenter =
					  $parent_env_json->{params}{vsphere_datacenter} || '';

					if ( $parent_env_json->{params}{vsphere_ephemeral_datastores} ) {
						$parent_vsphere_datastores_ephemeral = join( ' ',
							@{ $parent_env_json->{params}{vsphere_ephemeral_datastores} } );
					}

					if ( $parent_env_json->{params}{vsphere_persistent_datastores} ) {
						$parent_vsphere_datastores_persistent = join( ' ',
							@{ $parent_env_json->{params}{vsphere_persistent_datastores} } );
					}
				}
			}
		}
...
'What is the IP Address or Domain Name of your VMWare vCenter Server Appliance (:port optional)?',
		...
		'--validation',
'/^((([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])|(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9]))(?::[0-9]+)?$/'
	);

	my $vsphere_user =
	  prompt_for_line( 'What username should BOSH use to authenticate with vCenter?',
...
"Use the existing password for #M{$vsphere_user} at #C{$vsphere_address} from $ENV{BOSH_ALIAS} secrets?",
	...
	if ( $use_parent_password eq 'true' ) {
		system(
"safe --quiet set \"${self->env->secrets_base}vsphere\" password=\"$parent_vsphere_password\""
		);
	}
	else {
		my $vsphere_password =
		  prompt_for_line( 'What is the password for the vCenter user?', '--secret-line' );
		system(
			"safe --quiet set \"${self->env->secrets_base}vsphere\" password=\"$vsphere_password\""
		);
	}

	system(
"safe --quiet set \"${self->env->secrets_base}vsphere\" user=\"$vsphere_user\" address=\"$vsphere_address\""
	);

	my $vsphere_dc = prompt_for_line( 'What vCenter data center do you want to BOSH to deploy to?',
		'--default', $parent_vsphere_datacenter );

	my $vsphere_cluster = prompt_for_line( 'What vCenter cluster do you want BOSH to deploy to?',
		'--default', $parent_vsphere_cluster );

	my $vsphere_resource_pool =
	  prompt_for_line( 'What vCenter resource pool (if any) do you want BOSH to deploy to?',
		'--default', 'none' );
	...
	if ( $parent_vsphere_datastores_ephemeral || $parent_vsphere_datastores_persistent ) {
		...
		$use_parental_datastores = prompt_for_boolean(
			"Use the same persistent and ephemeral data stores as $ENV{BOSH_ALIAS} ?", '-i',
			'--default',                                                               'true'
		);
	}
	...
	if ($is_proto) {
		my $vsphere_net =
		  prompt_for_line('What is the name of the VM network in vCenter to deploy BOSH to?');
...
sub configure_google {
	...
	my $google_project_id = prompt_for_line('What is your GCP project ID?');

	my $google_json_key =
	  prompt_for_block('What are your GCP credentials (generally supplied as a JSON block)?');
	...
	if ($is_proto) {
		my $google_net = prompt_for_line('What is your GCP network name?');

		my $google_subnet = prompt_for_line('What is your GCP subnetwork name?');

		my $google_azs =
		  prompt_for_line('What availability zone do you want the BOSH VM to reside in?');

		my @google_tags =
		  prompt_for( 'multi-line', 'What tags would you like to be set on the BOSH VM?' );
...
sub configure_azure {
	...
	my $azure_client_id = prompt_for_line('What is your Azure Client ID?');

	my $azure_client_secret =
	  prompt_for_line( 'What is your Azure Client Secret?', '--secret-line' );

	my $azure_tenant_id = prompt_for_line('What is your Azure Tenant ID?');

	my $azure_subscription_id = prompt_for_line('What is your Azure Subscription ID?');

	my $azure_resource_group =
	  prompt_for_line('What Azure Resource Group will BOSH be deploying VMs into?');

	system(
"safe --quiet set \"${self->env->secrets_base}azure\" client_id=\"$azure_client_id\" client_secret=\"$azure_client_secret\" tenant_id=\"$azure_tenant_id\" subscription_id=\"$azure_subscription_id\""
	);

	my $azure_default_sg =
	  prompt_for_line('What security group should be used as the BOSH default security group?');

	$self->{config}{params}{azure_resource_group} = $azure_resource_group;
	$self->{config}{params}{azure_default_sg}     = $azure_default_sg;

	if ($is_proto) {
		my $azure_vnet = prompt_for_line('What is the name of your Azure Virtual Network?');

		my $azure_subnet_name =
		  prompt_for_line('What is the name of the Azure subnet that the BOSH will be placed in?');
...
sub configure_openstack {
	...
	my $openstack_auth_url = prompt_for_line('What is the Auth URL of your OpenStack cluster?');

	my $openstack_user =
	  prompt_for_line('What username will be used to authenticate with OpenStack?');

	my $openstack_password =
	  prompt_for_line( 'What password will be used to authenticate with OpenStack?',
		'--secret-line' );

	my $openstack_domain = prompt_for_line('What OpenStack Domain will BOSH be deployed in?');

	my $openstack_project = prompt_for_line('What OpenStack Project will BOSH be deployed in?');

	system(
"safe set --quiet \"${self->env->secrets_base}openstack/creds\" username=\"$openstack_user\" password=\"$openstack_password\" domain=\"$openstack_domain\" project=\"$openstack_project\""
	);

	my $openstack_region = prompt_for_line('What OpenStack Region is BOSH being deployed to?');
...
'What is the name of the OpenStack SSH key that should be used to enable SSH access to BOSH-deployed VMs?'
	...
	if ($is_proto) {
		my $openstack_network_id =
		  prompt_for_line('What is the UUID of the OpenStack network that BOSH will be placed in?');

		my $openstack_flavor =
		  prompt_for_line('What OpenStack flavor (instance type) should the BOSH VM use?');

		my $openstack_az = prompt_for_line('What AZ will the BOSH Director be placed in?');
...
sub configure_stackit {
	...
	my $stackit_auth_url = prompt_for_line('What is the Auth URL of your STACKIT cluster?');

	my $stackit_user = prompt_for_line('What username will be used to authenticate with STACKIT?');

	my $stackit_password =
	  prompt_for_line( 'What password will be used to authenticate with STACKIT?',
		'--secret-line' );

	my $stackit_domain = prompt_for_line('What STACKIT Domain will BOSH be deployed in?');

	my $stackit_project = prompt_for_line('What STACKIT Project will BOSH be deployed in?');

	system(
"safe set --quiet \"${self->env->secrets_base}stackit/creds\" username=\"$stackit_user\" password=\"$stackit_password\" domain=\"$stackit_domain\" project=\"$stackit_project\""
	);

	my $stackit_region = prompt_for_line('What STACKIT Region is BOSH being deployed to?');
...
'What is the name of the STACKIT SSH key that should be used to enable SSH access to BOSH-deployed VMs?'
	...
	if ($is_proto) {
		my $stackit_network_id =
		  prompt_for_line('What is the UUID of the STACKIT network that BOSH will be placed in?');

		my $stackit_subnet_id =
		  prompt_for_line('What is the UUID of the STACKIT subnet that BOSH will be placed in?');

		my $stackit_flavor =
		  prompt_for_line('What STACKIT flavor (instance type) should the BOSH VM use?');

		my $stackit_az = prompt_for_line('What AZ will the BOSH Director be placed in?');
...
sub get_blobstore_config {
	...
	my $blobstore = prompt_for_select(
		'What Blobstore do you want to use?',                          '-o',
		'[internal] Create a local blobstore on the BOSH director VM', '-o',
		'[s3]       Use an existing AWS S3 Blobstore',                 '--default',
		"internal"
	);
	...
	if ( $blobstore eq 's3' ) {
		...
		my $blobstore_s3_bucket = prompt_for_line('What is the name of the S3 Bucket?');

		my $blobstore_s3_region =
		  prompt_for_line( 'What region contains the \'' . $blobstore_s3_bucket . '\' S3 Bucket?' );
		...
		if ( $self->{config}{iaas} eq 'aws' ) {
			$blobstore_s3_auth_method = prompt_for_select(
				...
				'-o',
				'[access_key] Access/Secret Keypair',
				'-o',
				'[profile]    IAM Instance Profile',
				'--default',
				$self->{config}{aws_auth_method} || 'access_key'
			);
		}
		...
		if ( $blobstore_s3_auth_method eq 'access_key' ) {
			...
			system(
"safe set --quiet \"${self->env->secrets_base}blobstore/s3\" access_key=\"$access_key\" secret_key=\"$secret_key\""
			);
		}
		else {
...
sub get_dns_config {
	...
	my $dnshealthcheck = prompt_for_boolean( 'Do you want to enable BOSH DNS healthcheck?',
		'-i', '--default', 'true' );
	...
	my $dnscache =
	  prompt_for_boolean( 'Do you want to enable BOSH DNS caching?', '-i', '--default', 'true' );
...
sub get_access_config {
	...
	my $do_netop = prompt_for_boolean( "Enable the netop account? ", '-i', '--default', 'true' );
	...
	my $do_sysop = prompt_for_boolean( "Enable the sysop account? ", '-i', '--default', 'true' );
	...
	if ( $self->{config}{iaas} eq 'aws' ) {
		...
		if ($is_proto) {
			...
			if ( $self->{config}{params}{aws_proto_iam_instance_profile} ) {
				$output .=
"  aws_proto_iam_instance_profile: $self->{config}{params}{aws_proto_iam_instance_profile}\n";
			}
		}
	}
		...
		if ($is_proto) {
			...
			$output .=
			  "  google_subnetwork_name:   $self->{config}{params}{google_subnetwork_name}\n";
			$output .=
			  "  google_availability_zone: $self->{config}{params}{google_availability_zone}\n";
			$output .= "  google_tags:\n";
