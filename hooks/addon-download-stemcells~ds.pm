...
sub cmd_details {
	return "Download BOSH stemcells to a local directory. Supports the following options " .
	  "and arguments:\n" .
...
sub perform {
	...
	my %options = $self->parse_options(
		[ 'dir=s', 'os=s', 'light', 'regular', 'dry-run', ],
		os  => 'ubuntu-jammy',
		dir => '.',
	);
	...
	if ( !-d $options{dir} ) {
		if ( -e $options{dir} ) {
			bail( "Specified download directory '%s' exists but is not a directory",
				$options{dir} );
		}
		...
		if ( $options{'dry-run'} ) {
			...
			mkdir( $options{dir} )
			  or bail( "Failed to create directory '%s': %s", $options{dir}, $! );
		}
	}
	...
	if ( @versions == 0 ) {
		...
		info(
			" #%s{found %s}",
			scalar(@available) ? 'g' : 'r',
			count_nouns( scalar(@available), 'stemcell' )
		);
		if ( !@available ) {
			bail( "No available %s %s stemcells found for %s VMs",
				$type, $options{os}, $self->env->iaas );
		}
		...
		while ($again) {
			...
			if ($selected) {
				...
				$again =
				  prompt_for_boolean( "\nDo you want to select another stemcell? [y|n] ", 0 );
			}
			else {
				$again = 0;
			}
		...
		for my $stemcell (@stemcells) {
			info(
				...
				$stemcell->description(
					csprintf( "#Mu{Downloading stemcell %s:}n", $stemcell->{name} )
				) =~ s/\s+$//r
			);
			...
			if ( $options{'dry-run'} ) {
				...
				my ( $out, $rc, $err ) =
				  run( 'curl -L --progress-bar "%s" -o "%s"', $url, $target_path );
				...
				if ( -f $target_path ) {
					...
					info( "#g{Successfully downloaded stemcell to: %s (%s)}",
						$target_path, $size_mb );
				}
				else {
