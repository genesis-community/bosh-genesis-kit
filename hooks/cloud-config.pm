					...
					dynamic_subnets => {
						...
						cloud_properties_for_iaas => {
							aws => {
								...
								]    #$self->subnet_reference('sgs', 'get_security_groups'),
							},
							openstack => {
								'net_id' => $self->network_reference('id')
								,    # TODO: $self->subnet_reference('net_id'),
								'security_groups' => ['default'
								]    #$self->subnet_reference('sgs', 'get_security_groups'),
							},
							stackit => {
								'net_id'          => $self->network_reference('id'),
								'security_groups' => $self->network_reference(
									'sgs', 'get_sgs_by_names', 'ocfp', 'default'
								)
							},
						},
					},
				...
				$self->disk_type_definition(
					...
					common => {
						disk_size => $self->for_scale(
							{    # add $self->for_feature('internal-blobstore')
								dev  => gigabytes(32),
								prod => gigabytes(256)
							},
							gigabytes(32)
