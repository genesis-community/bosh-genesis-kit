...
sub perform {
	...
	for my $feature ( $self->features ) {
		bail( "Cannot specify a virtual feature: please specify $feature without the " .
	...
	if ( $self->iaas eq 'aws' ) {
		$self->add_feature( '+aws-secret-access-keys',
			!$self->has_feature('iam-instance-profile') );
		if ( $self->has_feature('s3-blobstore') ) {
			$self->add_feature( '+s3-blobstore-secret-access-keys',
				!$self->has_feature('s3-blobstore-iam-instance-profile') );
		}
	}
	else {
		$self->add_feature( '+s3-blobstore-secret-access-keys',
			$self->has_feature('s3-blobstore') );
	}

	if ( $self->has_feature('ocfp') ) {
		$self->add_feature( '+internal-blobstore', $self->has_feature('internal-blobstore') );
		bail("Invalid feature 'external-db-no-tls' while specifying 'internal-db' feature.")
		...
		bail("Invalid feature 'internal-db' without 'ocfp' feature.")
		  if $self->has_feature('internal-db');
		$self->add_feature( '+internal-blobstore',
			$self->has_feature('s3-blobstore') || $self->has_feature('minio-blobstore') );
		$self->add_feature( '+external-db',
			$self->has_feature('external-db-postgres') || $self->has_feature('external-db-mysql') );
	}

	# OCFP management gatekeeping
	if ( $self->has_feature('ocfp') && $self->env->name =~ /-mgmt$/ ) {
		bail( "Cannot deploy an OCFP management environment without " .
