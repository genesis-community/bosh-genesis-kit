package Genesis::Hook::Addon::BOSH::Resurrection v4.0.2;
...
sub perform {
	...
	if (@$args) {
		...
		if ($arg =~ /^(1|y|yes|t|true|on|enable.*)$/i) { # check for explicit true
			...
			bail("Expecting one of the following arguments: on (aka: yes|true|enabled|1) or off (aka: no|false|disabled|0)");
		}
	...
	if ($has_netop) {
		...
		bail("Cannot connect to %s using netop user -- skip-op-users feature is enabled", $env->name);
	} else {
		# Do it the slow way via BOSH SSH
		#my $call_with_env = $self->env->get_call_path_with_env();
		# TODO: can we use the above
		@ssh_cmd = ($env->get_call_path_with_env, "bosh", ($env->use_create_env ? () : ('--self')), "ssh", "-c");
	}
	...
	if ($paused eq 'true') {
		...
		my $deployed_state = $env->last_deployed_lookup("instance_groups[name=bosh].properties.hm.resurrector_enabled");
