...
sub before_terminate {
	...
	unless ( $self->use_create_env
		|| $self->parent_bosh->has_deployment( $self->env->deployment_name ) )
	{
		info(
		...
		if ( $self->{force} ) {
			...
			$continue =
			  prompt_for_boolean( "$msg\nDo you want to continue with the termination process?",
				"n" );
		}
	...
	info( "#%s{%s} - %s", $status->{status} eq 'ok' ? 'g' : 'r', $status->{status},
		$status->{msg} );
...
"These deployments must be terminated before deleting this BOSH director deployments.",
		...
		return $self->done(0)
		  ;    # TODO: Should this just bail, or return a complex structure with result and message?
	}
	else {
...
sub _after_terminate {
	...
	if ( $self->is_dryrun ) {
		dryrun( "Would have cleaned up network definition and claims in #M{%s/%s} exodus data",
			$self->env->name, $self->env->type, );
	}
	else {
		info( "Cleaning up network definition and claims in #M{%s/%s} exodus data",
			$self->env->name, $self->env->type );
